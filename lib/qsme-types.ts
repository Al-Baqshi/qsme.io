...
export type StructuredExtractionItem = {
  /** Stable region id (e.g. r0, r1). Fallback to index if missing. */
  id?: string
  page_id: string
  page_number: number
  bbox: NormalizedBbox
  ...
  table?: string[][]
  layout_label?: string
  /** Table regions: title, headers, rows, markdown, html */
  title?: string
  headers?: string[]
  rows?: string[][]
  markdown?: string
  html?: string
  /** Image/figure regions: figureIndex and optional image_url */
  figureIndex?: number
  image_url?: string
}
