...
class QuantityEngine:
    ...
    def compute(
        ...
        for room in rooms:
            ...
            door_width_sum = sum(self._opening_width_m(o, mx, my, engine_input) for o in room_openings if o.openingType == OpeningType.door)
            opening_area_sum = sum(self._opening_area_m2(o, mx, my, engine_input) for o in room_openings)
            ...
            qv_area = QuantityValue(value=area_m2, unit="m2", confidence={"score": conf})
            qv_perimeter = QuantityValue(value=perimeter_m, unit="m", confidence={"score": conf})
            qv_skirting = QuantityValue(value=skirting_m, unit="m", confidence={"score": conf})
            qv_wall_gross = QuantityValue(value=wall_area_gross, unit="m2", confidence={"score": conf})
            qv_wall_net = QuantityValue(value=wall_area_net, unit="m2", confidence={"score": conf})
            ...
            self._add_room_schedule_rows(schedule_rows, room, overlay_ids, qv_area, qv_perimeter, qv_skirting, qv_wall_gross, qv_wall_net)
    ...
    def _fill_missing_scales_from_measurements(self, scales: dict[UUID, PageScale], measurements: list[MeasurementOverlay]) -> None:
        for m in measurements:
            ...
            scales[m.pageId] = PageScale(
                ...
                confidence={"score": 0.8, "reason": "derived from measurement overlay"},
            )

    def _polygon_area_m2(self, points: list[NormalizedPoint], meters_per_norm_x: float, meters_per_norm_y: float) -> float:
        if len(points) < 3:
            return 0.0
    ...
    def _polygon_perimeter_m(self, points: list[NormalizedPoint], meters_per_norm_x: float, meters_per_norm_y: float) -> float:
        if len(points) < 2:
            return 0.0
        perimeter = 0.0
    ...
    def _opening_width_m(self, opening: OpeningOverlay, meters_per_norm_x: float, meters_per_norm_y: float, engine_input: QuantityEngineInput) -> float:
    ...
    def _opening_height_m(self, opening: OpeningOverlay, meters_per_norm_y: float, engine_input: QuantityEngineInput) -> float:
    ...
    def _opening_area_m2(self, opening: OpeningOverlay, meters_per_norm_x: float, meters_per_norm_y: float, engine_input: QuantityEngineInput) -> float:
        ...
        for trade, item, key, value, unit in total_defs:
            schedule_rows.append(
                QuantityScheduleRow(
                    ...
                    level=level,
                    overlayIds=[],
                    confidence={"score": 1.0, "reason": "deterministic aggregate"},
                )
            )
